...
{	
    ...
    [decodedValue appendBytes:eol length:1];
	
	data->offset += [dataLength integerValue]; // Always move the offset off the end of the encoded item.

    GEBEncodedType dataType = GEBEncodedDataType;
    if (data->typeAdvisor != nil)
        dataType = data->typeAdvisor(data->keyStack);
    long long int	number;
    NSString *parsedNumberString;

    
    switch (dataType) {
        case GEBEncodedStringType:
            return [NSString stringWithCString:[decodedValue bytes] encoding:NSUTF8StringEncoding];
            break;
        case GEBEncodedDataType:
            return decodedValue;
            break;
        case GEBEncodedNumberType:
            parsedNumberString=[NSString stringWithCString:[decodedValue bytes] encoding:NSUTF8StringEncoding];
            if (![[NSScanner scannerWithString:parsedNumberString] scanLongLong:&number]) 
            {
                NSLog(@"invalid number: %@",parsedNumberString);
                return nil;
            }
            return [NSNumber numberWithLongLong:number];
            break;
        default:
            NSLog(@"invalid type: %d",dataType);
            return nil;
    }
}
...
{
	return [BEncoding objectFromEncodedData:sourceData withTypeAdvisor:nil];
}
